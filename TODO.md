...
- [x] [APK] Mobile backup restore no longer freezes/corrupts: the restore runs on a background thread (instead of blocking the UI) and deletes the Room WAL/SHM sidecar files before overwriting `song.db`, so a restored DB is never mixed with stale journal frames on the next launch (which forced uninstall/reinstall). Playback is stopped before the DB is touched.
