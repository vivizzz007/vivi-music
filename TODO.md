...
- [x] Volume sync (two channels): the in-app player volume (`volume`) mirrors the mobile/desktop slider pixel-for-pixel, and the native OS system volume (`systemVolume`) mirrors Android STREAM_MUSIC <-> desktop OS volume (WinMM on Windows, `pactl`/`amixer` on Linux, `osascript` on macOS — all best-effort). Both sides poll volume (700ms mobile / 500+800ms desktop) with per-field echo guards and **retry on a dropped push** (a push that lands in the echo-suppression window is re-sent on the next tick instead of being lost), so volume syncs reliably even while idle. Windows opens the default wave device before reading/writing volume via the real `waveOutOpen` export (no A/W suffix — `waveOutOpenW` doesn't exist in `winmm.dll`, so looking it up threw "Error looking up function" and crashed the app); every native call is now guarded against missing symbols.
