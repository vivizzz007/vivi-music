...
- [x] Backup & restore sub-menu: exports/imports a full ZIP backup (settings + playlists + account/login + library via `BackupManager`) through a native save/open dialog; legacy single-JSON `.backup` files are still importable. Import preserves the device id + first-launch date, drops any stale pairing, and prompts a restart to apply.
