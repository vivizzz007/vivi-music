...
- [x] Volume sync (two channels): the in-app player volume (`volume`) mirrors the mobile/desktop slider pixel-for-pixel, and the native OS system volume (`systemVolume`) mirrors Android STREAM_MUSIC <-> desktop OS volume (WinMM on Windows, `pactl`/`amixer` on Linux, `osascript` on macOS — all best-effort). Both are pushed on change with echo suppression (broadcast on mobile, polling + guard on desktop). The desktop in-app volume is polled (not event-driven) so it also syncs while idle, and Windows opens the default wave device via `waveOutOpen` before reading/writing volume (the old code passed `WAVE_MAPPER` as a handle, which always failed).
