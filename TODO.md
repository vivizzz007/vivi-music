...
- [x] Resolving/buffering handshake: `PlaybackSnapshot.isResolving` is set by the desktop while it resolves/downloads (until the first position report = audio actually flowing) and by the mobile while ExoPlayer is `STATE_BUFFERING`. The receiver prepares the queue but holds playback (and `effectivePosition` stops extrapolating a frozen position), so neither device starts a track ahead of the other while the peer is still resolving. Mobile pushes the buffering→ready transition promptly (not just on the 5s tick). Resolving transitions bypass the echo-suppression window on both sides (they were being swallowed by the 1.5s window right after applying a remote snapshot, so a phone-initiated track change played ahead of the still-resolving desktop). The desktop no longer `startPaused`-holds on the peer's resolving state (it is the slower device and the phone holds for it), and it ignores the peer's play/pause echoes while it is itself resolving — this fixed a startup deadlock where both paused and the desktop stayed silent (in the mixer but emitting nothing).
...
- [x] Volume sync (two channels): the in-app player volume (`volume`) mirrors the mobile/desktop slider pixel-for-pixel, and the native OS system volume (`systemVolume`) mirrors Android STREAM_MUSIC <-> desktop OS volume (WinMM on Windows, `pactl`/`amixer` on Linux, `osascript` on macOS — all best-effort). The in-app volume channel can now be disabled independently with the "Sync VIVI volume" toggle (Settings → Devices and Settings → Player & audio on both editions); the native OS volume sync stays separate and unaffected. Both sides poll volume (700ms mobile / 500+800ms desktop) with per-field echo guards and **retry on a dropped push** (a push that lands in the echo-suppression window is re-sent on the next tick instead of being lost), so volume syncs reliably even while idle. Windows now drives the **master** volume through WASAPI `IAudioEndpointVolume` (Core Audio COM marshaled onto a dedicated MTA thread) and pins the app's own session (`VIVIMusic` in the mixer) to 100% — WinMM `waveOut*` only moved the per-app session volume, so it was replaced; every native call is guarded against missing symbols. The "Sync VIVI volume" toggle itself is now part of the shared settings snapshot (read/pushed/apply on both editions), so flipping it on one device mirrors on the other.
