...
- [x] Volume sync (two channels): the in-app player volume (`volume`) mirrors the mobile/desktop slider pixel-for-pixel, and the native OS system volume (`systemVolume`) mirrors Android STREAM_MUSIC <-> desktop OS volume (WinMM on Windows, `pactl`/`amixer` on Linux, `osascript` on macOS — all best-effort). Both are pushed on change with echo suppression (broadcast on mobile, polling + guard on desktop).
...
- [~] Song / album / artist / playlist context menus port. The full song context menu (like / library / add-to-playlist / share) is done, plus a dedicated "Add to playlist" button on every song row; album / artist / playlist context menus are still pending.
