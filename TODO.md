...
- [x] [APK] Mobile backup restore is safe: the archive is decompressed on a background thread and staged to temp files, then settings + database are swapped in and the process is killed in one synchronous main-thread block (no window for the UI to query a closed DB). The Room WAL/SHM sidecars are deleted before swapping `song.db` so the restored DB is never mixed with stale journal frames on the next launch (which forced uninstall/reinstall). Playback is stopped before the DB is touched.
